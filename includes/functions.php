...
{	
	...
	if ($f_id > 0)
	{
		$sql = 'SELECT forum_name
				...
				WHERE forum_id = ' . $f_id;
		$result = $db->sql_query($sql);
		$title = (string) $db->sql_fetchfield('forum_name');
		$db->sql_freeresult($result);
		
		$new_url .= title_to_url($title) . '-f' . $f_id;
	}
	
	// Add topic_title from topic
	if ($t_id > 0)
	{
		$new_url .= ($new_url == '') ? '' : '/';
		...
		$sql = 'SELECT topic_title
				...
				WHERE topic_id = ' . $t_id;
		$result = $db->sql_query($sql);
		$title = (string) $db->sql_fetchfield('topic_title');
		$db->sql_freeresult($result);
		
		if ($p_id == 0)
		{
			$new_url .= title_to_url($title) . '-t' . $t_id;
		}
		else 
		{
...
function get_fecha_rol() {	
	$initialDate = new DateTime('2019-01-01');
	$initialYear = 144;
	...
	$season = $seasons[(int) $now->format('m') - 1];
	$day = (int) $now->format('d') * 3;
	...
	$diff = date_diff($initialDate, $now);
	$year = $initialYear + ((int) $diff->y * 3) + floor((int) $diff->m / 4);
	
	//return sprintf("diff_y %d - diff_m %d", (int)$diff->y, (int)$diff->m);
