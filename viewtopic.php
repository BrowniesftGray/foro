...
$s_quickmod_action = append_sid(
	...
	array(
		...
		'start'		=> $start,
		'quickmod'	=> 1,
		'redirect'	=> urlencode(str_replace('&amp;', '&', str_replace('//', '/', str_replace('.', '', $viewtopic_url)))),
	),
	true,
	$user->session_id
);
...
$template->assign_vars(array(
	...
	'TOPIC_SUBTITLE'=> censor_text($topic_data['topic_subtitle']),	// mgomez // 11-12-2018
...
for ($i = 0, $end = count($post_list); $i < $end; ++$i)
{
	...
	*/
	...
	$post_row = array(
		...
		'PJ_DIFF_PV'		=> abs((int)$row['diff_pv']),
		'PJ_DIFF_PC'		=> abs((int)$row['diff_pc']),
		'PJ_DIFF_STA'		=> abs((int)$row['diff_sta']),
		'PJ_SIGNO_PV'		=> ((int)$row['diff_pv'] > 0 ? '+' : '-'),
		'PJ_SIGNO_PC'		=> ((int)$row['diff_pc'] > 0 ? '+' : '-'),
		'PJ_SIGNO_STA'		=> ((int)$row['diff_sta'] > 0 ? '+' : '-'),
