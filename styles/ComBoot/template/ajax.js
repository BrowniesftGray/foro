...
phpbb.markNotifications = function($popup, unreadCount) {
	...
	if (!unreadCount) {
		$('#notification_list .card-body, #notification_list_mobile .card-body').remove();
	}
...
phpbb.addAjaxCallback('vote_poll', function(res) {
	if (typeof res.success !== 'undefined') {
		...
		poll.find('.polls, .poll_max_votes, .poll_vote').show(500, function () {
			poll.find('.vote_option').removeClass().addClass('col-md-3 offset-1 vote_option');
...
/**
* Toggle the member search card in memberlist.php.
*
* If user returns to search page after viewing results the search card is automatically displayed.
* In any case the link will toggle the display status of the search card and link text will be
* appropriately changed based on the status of the search card.
*/
