...
$(document).on('ready pjax:success', function() {
			...
			if (!$this.is(':empty')) {
				$this.children('a').each(function() {
					var $link = $(this),
						...
						$href = $link.attr('href'),
						$item = '<a class="dropdown-item" href="' + $href + '"><i class="' + $icon + '" aria-hidden="true"></i> ' + $title + '</a>';
					$postbody.find('.btn-toolbar-mobile ul').append($item);
				});
				if (index !== $btnGroupsAmount - 1) {
					$postbody.find('.btn-toolbar-mobile ul').append('<div class="dropdown-divider"></div>');
				}
				$postbody.find('.btn-toolbar-mobile').removeClass('hidden').addClass('d-block d-sm-none');
			}
		});
	});
	...
	if ($('#footer-nav .nav.ml-auto').length === 0) {
		$('#footer-nav ul.navbar-nav').after('<ul class="nav navbar-nav ml-auto"><li class="nav-item">span class="navbar-text" id="copyright">Design: <a href="http://zoker.me/go/comboot" target="_blank">ComBoot</a> by <a href="http://www.florian-gareis.de" target="_blank" id="author-name">Florian Gareis</a></span></li></ul>');
	}
	...
	var $copy = $('#footer-nav .nav.ml-auto .nav-item').html();
	$('#page-footer .copyright').prepend('<div class="d-block d-sm-none">' + $copy + '</div>');
...
$(document).on('click', '.card-header span.clickable', function(e) {
	...
	if (!$this.hasClass('card-collapsed')) {
		$this.parents('.card-collapsible').find('.card-body, .card-footer').slideUp();
		$this.addClass('card-collapsed');
		$this.find('i').removeClass('fa-chevron-up').addClass('fa-chevron-down');
	} else {
		$this.parents('.card-collapsible').find('.card-body, .card-footer').slideDown();
